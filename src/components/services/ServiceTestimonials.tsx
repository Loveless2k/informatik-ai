  ...
  return (
    <section className={`py-24 md:py-32 relative overflow-hidden ${
      ...
      <div className={`absolute inset-0 ${
        isDarkMode
          ? 'bg-grid-white/[0.04]' : 'bg-grid-slate-300/[0.2]'} bg-[length:30px_30px]`}></div>
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <SectionHeading
          title={<span className="text-transparent bg-clip-text bg-gradient-to-r from-[#00B4DB] via-[#48D1CC] to-[#00BFFF] font-bold">Casos de Éxito</span>}
          subtitle={<span className={`${isDarkMode ? 'text-gray-300' : 'text-gray-600'} max-w-2xl mx-auto text-base md:text-lg`}>Resultados reales que hemos logrado para nuestros clientes</span>}
          centered
        />
          ...
          {successCases.map((caseStudy, index) => (
            ...
            >
              <div className="p-6 md:p-8">
                <span className={`inline-block px-3 py-1 rounded-full mb-4 text-sm font-semibold ${
                  isDarkMode
                    ? 'bg-blue-500/20 text-blue-300'
                    : `${caseStudy.tagBg.light} ${caseStudy.tagText.light}`
                }`}>{caseStudy.industry}</span>
                  ...
                  {caseStudy.results.map((result, i) => (
                    <li key={i} className="flex items-start">
                      <svg className={`w-5 h-5 mr-2 mt-1 flex-shrink-0 ${
                        isDarkMode ? 'text-green-400' : caseStudy.resultIcon.light
                      }`} fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7"></path>
                      </svg>
                      <span className={`${isDarkMode ? 'text-blue-100' : 'text-slate-700'}`}>{result}</span>
                    </li>
                  ))}
                </ul>
              </div>
        ...
        >
          <p className={`text-base md:text-lg max-w-2xl mx-auto leading-relaxed ${isDarkMode ? 'text-blue-100' : 'text-slate-700'}`}>
            Estos son solo algunos ejemplos de cómo nuestras soluciones han generado un impacto real y medible en diferentes industrias.
            Adaptamos cada estrategia para lograr resultados excepcionales en cada organización.
          </p>
